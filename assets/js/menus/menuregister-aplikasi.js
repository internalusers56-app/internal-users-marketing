// File: assets/js/menus/menuregister-aplikasi.js

function initPage() {
    console.log("Register Aplikasi page initialized.");
    ...
    const tableBody = document.querySelector('#register-aplikasi-table tbody');

    // --- ELEMEN DOM UNTUK PENCARIAN LANJUTAN ---
    const advancedSearchBtn = document.getElementById('advanced-search-btn');
    const resetSearchBtn = document.getElementById('reset-search-btn'); // Tombol Batal
    ...
    // --- EVENT LISTENER UNTUK TOMBOL BATAL (RESET FILTER) ---
    ...
    function advancedFilterTable() {
        ...
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            const rowDateText = row.cells[0].textContent; // Kolom Tanggal
            const rowStatus = row.cells[8].textContent;   // Kolom Result
    ...
    tableBody.addEventListener('click', (event) => {
        ...
        if (target.classList.contains('btn-update')) {
            ...
            const noRequest = row.cells[6].textContent;
            console.log(`Update data untuk No. Request: ${noRequest}`);
            alert(`Fitur update untuk No. Request: ${noRequest} (Simulasi)`);
        }
        if (target.classList.contains('btn-delete')) {
            const row = target.closest('tr');
            const noRequest = row.cells[6].textContent;
            if (confirm(`Apakah Anda yakin ingin menghapus data dengan No. Request: ${noRequest}?`)) {
                console.log(`Hapus data untuk No. Request: ${noRequest}`);
                row.remove();
                alert(`Data dengan No. Request: ${noRequest} telah dihapus (Simulasi)`);
            }
        }
    });
